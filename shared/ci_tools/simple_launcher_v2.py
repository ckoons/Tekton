"""
Simple Tool Launcher V2 - Uses C launcher in stdio mode only.
"""
...
class SimpleToolLauncherV2:
    """
    Simple tool launcher that uses C launcher for process management.
    No singletons, no complex state - just launch and track via PID files.
    """
    ...
    def launch_tool(self, tool_name: str, session_id: Optional[str] = None, instance_name: Optional[str] = None) -> bool:
        """
        Launch a CI tool using C launcher in stdio mode.
        """
        ...
        try:
            # Launch with C launcher in background
            self.logger.info(f"Launching {tool_name} with C launcher: {' '.join(cmd)}")
            
            # Use subprocess.Popen to launch with stdin from devnull to keep it open
            with open(os.devnull, 'r') as devnull:
                process = subprocess.Popen(
                    cmd,
                    env=env,
                    stdin=devnull,  # Use devnull to keep stdin open but not blocking
            ...
            if process.poll() is not None:
                # Process exited
                self.logger.error(f"Tool exited immediately with code: {process.poll()}")
                return False
            
            # Write PID file
            self._write_pid_file(launch_key, process.pid, port, session_id)
            self.logger.info(f"Wrote PID file for {launch_key} (PID: {process.pid})")
