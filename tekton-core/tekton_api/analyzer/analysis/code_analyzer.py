...
class CodeAnalyzer:
    ...
    def __init__(self, llm_client=None):
        ...
        Args:
            llm_client: LLM client for text generation (optional, will use TektonCore's LLM client if not provided)
        """
        self.llm_client = llm_client
        if not self.llm_client:
            # TODO: Initialize with TektonCore's LLM client when available
            logger.warning("No LLM client provided - some functionality will be limited")
    ...
    async def extract_parameters(
        ...
        """
        if not self.llm_client:
            logger.warning("No LLM client available for parameter extraction")
...
"""
    ...
    async def extract_capabilities(
        ...
        """
        if not self.llm_client:
            logger.warning("No LLM client available for capability extraction")
