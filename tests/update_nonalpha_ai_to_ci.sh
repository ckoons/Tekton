#!/bin/bash
# Script to replace "CI " with "CI " when CI is preceded by a non-alphabetic character
# This catches patterns like " CI ", "[CI ", "(CI ", "-CI ", etc.
...
echo -e "${GREEN}Starting CI to CI conversion (non-alpha prefix pattern)...${NC}"
echo "Pattern: [^a-zA-Z]CI → [^a-zA-Z]CI"
echo "----------------------------------------"
...
find . -type f \
    ...
    # Check if file contains the pattern (non-alpha char followed by "CI ")
...
echo "  3. Search for remaining patterns: grep -rE '[^a-zA-Z]CI ' . --exclude-dir=.git --exclude-dir=backup_*"
